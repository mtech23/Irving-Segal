        ...
        <>
            <AuthLayout authTitle='Verification Code' authPara='Please Check Your Email For Verification Code.' subauthPara='Your Cod is 4 digit in Length' backOption={true}>
                <form>
                    ...
                    <CustomInput required id='userEmail' type='text' placeholder='Verification Code' labelClass='mainLabel' inputClass='mainInput' onChange={(event) => {
                        setFormData({ ...formData, password: event.target.value })
                    }} /> <p className='otpclass'>{otp}</p>
                    {/* </div> */}
                    <div className="d-flex align-items-baseline justify-content-between mt-1">
                        <p className='text-danger fw-bold'>
                            {canResend ? 'You can resend the code now.' : `Resending in 00:${timer < 10 ? `0${timer}` : timer}`}
                        </p>                        <button
                            type='button'
                            className={`notButton primaryColor fw-bold text-decoration-underline ${!canResend && 'disabled'}`}
